...
SESSION_POST_REQ = endpoints.ResourceContainer(
    SessionForm,
    websafeConferenceKey=messages.StringField(1),
)

SESSION_WITH_POST_REQ = endpoints.ResourceContainer(
    message_types.VoidMessage,
    websafeConferenceKey=messages.StringField(1),
...
# - - - Registration - - - - - - - - - - - - - - - - - - - -
    ...
    def _getSession(self, request):
        ...
        for filtr in filters:
            if filtr["field"] in ["starttime", "duration"]:
                filtr["value"] = int(filtr["value"])
            formatted_query = ndb.query.FilterNode(filtr["field"], filtr["operator"], filtr["value"])
            q = q.filter(formatted_query)
        return q
    ...
    def _copySessionToForm(self, mysession):
        ...
        for field in cf.all_fields():
            if hasattr(mysession, field.name):
                # convert Date to date string; just copy others
                if field.name.endswith('Date'):
                    setattr(cf, field.name, str(getattr(mysession, field.name)))
                else:
    ...
    @endpoints.method(SESSION_POST_REQ, SessionForm, path='createsession/{websafeConferenceKey}',
            http_method='POST', name='createSession')
    def createSession(self, request):
        """Create new session"""
        return self._createSessionObject(request)

    @endpoints.method(SESSION_WITH_POST_REQ, SessionForms,
            path='session/{websafeConferenceKey}',
            http_method='POST',
            name='getConfSession')
    def getConfSession(self, request):
        """Query for Sessions."""
        my_key = ndb.Key(urlsafe=request.websafeConferenceKey)

        if not my_key.get():
            raise endpoints.NotFoundException(
                'No Session found with key:  %s' % request.websafeConferenceKey)

        Sessions = Session.query(ancestor=my_key)

         # return individual ConferenceForm object per Conference
        return SessionForms(
            items=[self._copySessionToForm(mysession) \
            for mysession in Sessions]
        )

    """ Query any requested filter """
    @endpoints.method(SessionQueryForms, SessionForms,
            path='querySessions',
            http_method='POST',
            name='querySessions')
    def querySessions(self, request):
        """Query for session with filters."""
        sessions = self._getSession(request)
    ...
    def getConferenceSessionsByType(self, request):
        ...
        Sessions = Session.query(ancestor=my_key).filter(Session.typeOfSession == request.typeOfSession)
    ...
    """Featured Speaker endpoint"""
    @endpoints.method(message_types.VoidMessage, StringMessage,
            path='getFeaturedSpeaker',
            http_method='GET', name='getFeaturedSpeaker')
    def getFeaturedSpeaker(self, request):
        """Get list of featured speaker"""
        return StringMessage(data=memcache.get(MEMCACHE_FEATURED_KEY) or "")

# - - - - - WishList - - - - - - - - - - - - - - 

    """ Register/ unregister session to wishlist """
    @ndb.transactional(xg=True)
    ...
    def addSessionToWishlist(self, request):
        """Add selected session to wishlist."""
    ...
    def getSessionsInWishlist(self, request):
        """Get list of session that user has added to widshlist"""
        ...
        # return set of ConferenceForm objects per Conference
        return SessionForms(items=[self._copySessionToForm(conf)\
         for conf in sessions]
        )

# - - - - - Queries - - - - - - - - - - - - - - - - - - - -
    """ Query session available in the morning only and that last between 60-90 minute . """
    @endpoints.method(message_types.VoidMessage, SessionForms,
            path='queryMorningSession',
            http_method='POST', name='queryMorningSession')
    def queryMorningSession(self, request):
        """Query morning sessions that last at least 60 minute """
        morning = Session.query(Session.starttime <= 12)
        start = Session.query(Session.duration >= 60)
        end = Session.query(Session.duration <= 90)

        sessions = []
        for session in morning:
            if session in start and end:
    ...
    """ Query Highlights about programming python """
    @endpoints.method(message_types.VoidMessage, SessionForms,
            path='queryHighlightsProgramming',
            http_method='POST', name='queryHighlightsProgramming')
    def queryHighlightsProgramming(self, request):
        """Query today sessions """
        sessions= Session.query(Session.highlights == "Programming python")
    ...
    @endpoints.method(message_types.VoidMessage, SessionForms,
            path='queryProblem',
            http_method='POST', name='queryProblem')
    def queryProblem(self, request):
        """Query Problem """

        mysessions = []
        sessiontype = Session.query(Session.typeOfSession != "workshop")
        sessiontime = Session.query(Session.starttime < 19)
        for session in sessiontype:
            if session in sessiontime:
                mysessions.append(session)

        return SessionForms(
            items=[self._copySessionToForm(sess) for sess in mysessions]
        )
